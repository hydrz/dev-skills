...
它会配置 issue 追踪器、分诊标签、领域文档布局和范围清单约定。之后如果不确定该使用哪个 skill，运行：
...
如果工作规模大到一个会话无法理清，使用 `/wayfinder` 将它拆成可以逐步解决的决策问题。交付新产品或子系统时，它还会建立范围清单，列出每个界面和能力的状态与设计依据；之后的规格、任务、评审和完成验证都按清单逐项核对，避免交付物只做了主链路、界面偏离设计。如果只是想在仓库外澄清一个计划，使用不写文件的 `/grill-me`。
